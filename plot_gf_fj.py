...
TIMINGS_DIR: Final[str] = os.path.join(FILE_DIR, "timings")
...
def read_job_results(algo: Algo) -> pd.DataFrame:
    job_data_dir: Final[str] = os.path.join(TIMINGS_DIR, f"{algo.value}_results")
    job_results: Final[str] = os.path.join(TIMINGS_DIR, f"{algo.value}_results.csv")
...
def read_free_join_results(algo: Algo) -> pd.DataFrame:
    with open(os.path.join(TIMINGS_DIR, f"{algo.value.lower()}.json")) as f:
        data = json.load(f)
